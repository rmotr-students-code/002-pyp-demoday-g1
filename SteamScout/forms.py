...
class SignUpForm(Form):
    username = TextField(
        ...
        validators=[Length(min=4, max=25,
                           message=(u'Username must be between 4 and 25 characters')),
    ...
    password = PasswordField(
        'Enter a Password',
        validators=[Required("Enter a secure password"),
...
class PercentPref(Form):
    threshold_percent = IntegerField(
        'percent threshold', validators=[Required("Please enter a number between 1-100"),
...
class AmountPref(Form):
    threshold_amount = DecimalField(
        'amount threshold',
        validators=[Required("Please enter an amount between .01 and 1000.00"),
...
class GamesSearch(Form):
    search_term = TextField(
        'Enter a Game',
        validators=[Required("Please enter a game to search for")])
